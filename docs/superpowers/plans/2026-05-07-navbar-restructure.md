# Navbar Restructure (B+ Pattern) — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Reemplazar el dropdown actual del navbar (8 items planos linkeando incorrectamente) por un dropdown estructurado en 4 secciones temáticas con 17 items, en los 17 archivos HTML que tienen `terra-nav` inline.

**Architecture:** Un script Python que recorre todos los archivos `*.html` bajo `public/` y aplica 3 transformaciones: (1) reemplaza el bloque HTML `<nav class="terra-nav">...</nav>`, (2) reemplaza el `<script>` IIFE que viene inmediatamente después, (3) actualiza el CSS del `.terra-nav__dropdown` (ancho + nuevas reglas para secciones). El `data-page="..."` específico de cada página se preserva.

**Tech Stack:** Python (regex + file I/O), HTML/CSS/JS inline, git, gh CLI.

---

## Pre-requisitos

- Repositorio clonado en `/tmp/viverosterra-site`
- Branch actual: `main`
- 17 archivos HTML contienen `terra-nav` (verificar con `grep -rl 'class="terra-nav"' /tmp/viverosterra-site/public/ --include="*.html" | wc -l`)

---

## Task 1: Crear y ejecutar script de reemplazo del navbar

**Files:**
- Create: `/tmp/viverosterra-site/scripts/replace-navbar.py`
- Modify: ~17 archivos HTML bajo `/tmp/viverosterra-site/public/`

---

- [ ] **Step 1: Crear el directorio scripts y el archivo Python**

```bash
mkdir -p /tmp/viverosterra-site/scripts
```

- [ ] **Step 2: Crear el script Python con la lógica de reemplazo**

Crear el archivo `/tmp/viverosterra-site/scripts/replace-navbar.py` con este contenido completo:
...
# ─────────────────────────────────────────────────────────────
# NUEVO HTML del nav (sin la línea <nav class="terra-nav" data-page="...">,
# eso se compone dinámicamente con el data-page que ya tiene cada archivo)
# ─────────────────────────────────────────────────────────────
NEW_NAV_INNER = '''
...
# ─────────────────────────────────────────────────────────────
# NUEVO SCRIPT JS — maneja múltiples sub-toggles
# ─────────────────────────────────────────────────────────────
NEW_SCRIPT = '''
<script>
(function() {
  ...
  // Manejar TODOS los sub-toggles del drawer (4 secciones)
...
# ─────────────────────────────────────────────────────────────
# NUEVAS REGLAS CSS — se añaden al CSS existente del .terra-nav
# ─────────────────────────────────────────────────────────────
NEW_CSS_RULES = '''
.terra-nav__dropdown {
...
# ─────────────────────────────────────────────────────────────
# REGEX para encontrar el bloque <nav>...</nav> y el <script> IIFE
# ─────────────────────────────────────────────────────────────
...
# El script IIFE que viene justo después del </nav>
SCRIPT_PATTERN = re.compile(
    r'<script>\s*\(function\(\) \{\s*var nav = document\.querySelector\(\'\.terra-nav\'\);[\s\S]*?\}\)\(\);\s*</script>',
    re.DOTALL
)

# Para añadir el CSS al final del bloque .terra-nav__cta
...
def process_file(path: pathlib.Path) -> bool:
    """Procesa un archivo HTML. Retorna True si se modificó."""
    try:
    ...
    # 1. Reemplazar el bloque <nav>...</nav>, preservando el data-page
    def nav_repl(match):
        opening_tag = match.group(1)  # <nav class="terra-nav" data-page="...">
    ...
    # 2. Reemplazar el script IIFE
    new_text, n = SCRIPT_PATTERN.subn(NEW_SCRIPT, text, count=1)
    if n > 0:
        changes.append(f'script JS ({n})')
        text = new_text

    # 3. Inyectar las nuevas reglas CSS — solo si aún no están presentes
    if '.terra-nav__drop-section' not in text:
        ...
        if n > 0:
            ...
            # Fallback: inyectar antes del cierre de </style> si el patrón no coincide
            if '</style>' in text:
...
- [ ] **Step 3: Ejecutar el script y verificar conteo de modificaciones**

```bash
cd /tmp/viverosterra-site
python3 scripts/replace-navbar.py
```

Expected output: lista de archivos con `✅` y al final `📊 Total modificados: 17`.

Si ves menos de 17, alguno de los archivos tiene el navbar en un formato ligeramente diferente. En ese caso, identifica el archivo con `grep -L 'terra-nav__drop-section' /tmp/viverosterra-site/public/**/*.html` y revisa manualmente.

- [ ] **Step 4: Verificar que las nuevas secciones existen en los archivos modificados**

```bash
# Cuántos archivos tienen las 4 secciones nuevas
grep -rl 'Servicios de Jardinería' /tmp/viverosterra-site/public/ --include="*.html" | wc -l
grep -rl 'Plantas y Palmas' /tmp/viverosterra-site/public/ --include="*.html" | wc -l
grep -rl 'Materiales</div>' /tmp/viverosterra-site/public/ --include="*.html" | wc -l
grep -rl 'Por Zona · B2B' /tmp/viverosterra-site/public/ --include="*.html" | wc -l
```

Expected: cada uno debe retornar **17** (todos los archivos con navbar).

- [ ] **Step 5: Verificar que las páginas huérfanas ahora son linkeables desde el navbar**

```bash
# Estas URLs ahora deben aparecer en los hrefs del navbar:
grep -rl 'href="/palmas-tropicales-tampico"' /tmp/viverosterra-site/public/ --include="*.html" | wc -l
grep -rl 'href="/arboles-para-banqueta-tampico"' /tmp/viverosterra-site/public/ --include="*.html" | wc -l
grep -rl 'href="/plantas-de-interior-tampico"' /tmp/viverosterra-site/public/ --include="*.html" | wc -l
grep -rl 'href="/vivero-tampico"' /tmp/viverosterra-site/public/ --include="*.html" | wc -l
grep -rl 'href="/jardineria-ciudad-madero"' /tmp/viverosterra-site/public/ --include="*.html" | wc -l
grep -rl 'href="/jardineria-altamira"' /tmp/viverosterra-site/public/ --include="*.html" | wc -l
grep -rl 'href="/mayoreo-pasto-tampico"' /tmp/viverosterra-site/public/ --include="*.html" | wc -l
```

Expected: cada uno debe retornar **17** o más.

- [ ] **Step 6: Commit y push**

```bash
cd /tmp/viverosterra-site
git add scripts/replace-navbar.py public/
git commit -m "feat(nav): restructurar dropdown en 4 secciones temáticas con 17 items

Antes: 8 items planos, 'Piedras y Mármol' linkeaba mal a tezontle-rojo
Ahora: 4 secciones (Servicios | Plantas | Materiales | Por Zona/B2B)
con 17 items distintos. Mobile drawer con 4 sub-toggles colapsables.

- 17 archivos HTML actualizados sistemáticamente vía script Python
- Resuelve link incorrecto de 'Piedras y Mármol'
- Añade al navbar las 4 páginas nuevas y 6 páginas huérfanas
- Dropdown ampliado de 580px a 720px con 2 columnas × 2 secciones"
git push origin main
```

- [ ] **Step 7: Verificación visual en producción**

Esperar ~2 min para que Vercel despliegue. Luego verificar en navegador:

```
https://www.viverosterra.com
```

Hacer click en "Servicios" del navbar y verificar que el dropdown muestra las 4 secciones con sus respectivos items. Probar también en móvil (resize a 375px) y verificar los 4 sub-toggles del drawer.
...
- ✅ 17 items en 4 secciones — definidos completos en `NEW_NAV_INNER`
- ✅ Layout 2 columnas × 2 secciones — `NEW_CSS_RULES` define `grid-template-columns: 1fr 1fr`
- ✅ Dropdown 720px — definido en `NEW_CSS_RULES`
- ✅ Headers de sección — clase `.terra-nav__drop-header` con estilo verde uppercase
- ✅ 4 sub-toggles en mobile drawer — cada uno con `data-target` único
- ✅ JS actualizado para `forEach` en sub-toggles
- ✅ Preserva `data-page` específico de cada página
- ✅ Top nav sin cambios (Catálogo y Blog quedan como están)
- ✅ Sin cambios al CSS existente del nav, solo se añaden reglas nuevas

### Placeholder scan

- ✅ Sin TBDs, TODOs ni referencias vagas
- ✅ Todo el código completo en cada step
- ✅ Comandos exactos con resultados esperados

### Type consistency

- ✅ IDs únicos para cada sub-menú: `drawer-sub-servicios`, `drawer-sub-plantas`, `drawer-sub-materiales`, `drawer-sub-zonas`
- ✅ Atributo `data-target` matchea con los IDs
- ✅ Selectores CSS consistentes (`.terra-nav__drop-section`, `.terra-nav__drop-header`, `.terra-nav__drop-col`)
- ✅ El script JS usa `querySelectorAll` para múltiples toggles, consistente con el HTML que tiene 4 instancias
